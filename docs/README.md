...
截至 2026-06-13，本仓库已核验到 `front/`、`backend/`、`docs/`、`tests/specs/` 和自动化一致性门禁。`front/` 是 React/Vite/TypeScript 前端工程，当前具备基础组件、反馈/状态组件、数据展示组件、业务组件、壳路由和 AI Studio 页面生成前上下文包；`backend/` 是 Go/Chi 模块化单体后端工程，OpenAPI 契约版本 `1.0.0` 已冻结，`front/src/types/api.ts` 已由契约自动生成。真实业务页面尚未生成，前端仍不得使用 Mock 数据、临时 DTO 或猜测字段推进页面。

文档中涉及部署、性能指标、上线状态、真实用户数据或端到端业务完成度的内容，仍应按“目标设计 / 待验证方案 / 待执行报告”处理，不能直接作为当前已完成事实。
...
5. [第六阶段 M3 页面生成前准备审核记录](phase-6-m3-page-generation-readiness-audit.md)
...
| `../README.md` | 项目入口和当前状态说明 | 记录前端、后端、契约、AI 输入包和下一步状态 |
| `../DESIGN.md` | 产品体验和设计 source of truth | 设计原则与当前组件/API 事实并存；不声明真实业务页面已完成 |
...
| `asset-baseline-investigation-report.md` | 当前源码资产事实调查 | 已更新为 front/backend 已存在、M1/M2 完成、M3 生成前准备状态 |
| `phase-4-technical-architecture-design.md` | 目标技术架构、模块、接口、安全和部署 | 目标架构设计；已落地部分以后端契约和源码为准 |
| `function-analysis-and-system-architecture.md` | 功能和系统架构分析 | 历史/目标分析文档；其中源码路径需按当前资产基线复核 |
| `phase-5-component-library-and-reuse-design.md` | 组件库、接口契约和 AI 生成前端衔接 | 执行方案；当前组件库和业务组件已进入生成前审核 |
| `phase-6-m3-page-generation-readiness-audit.md` | M3 页面生成前准备审核 | G2/G3 准入记录；不代表页面已生成 |
| `docker-compose-guide.md` | Docker/Compose 学习与后续部署指南 | 需按真实部署结果补最终命令与报告 |
...
- 源码、契约或生成页面状态变化后，应优先更新 `asset-baseline-investigation-report.md`，再更新架构、部署、性能和 README。
- 用于毕业设计论文或项目验收时，应以真实系统、真实测试记录、真实截图和学校模板为准。

## 下一步文档整理建议

1. 页面生成后增加“已实现 / 部分实现 / 未实现”功能矩阵。
2. M4 联调后增加真实接口对接说明、环境变量、错误边界和测试记录。
3. M5/M6 后补充真实运行命令、部署拓扑、性能报告和回滚方案。
