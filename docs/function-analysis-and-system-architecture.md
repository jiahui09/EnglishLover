...
> 当前项目依据：`README.md`、`DESIGN.md`、`front/src/types.ts`、`front/src/App.tsx`、`front/server.ts`、`front/src/components/*`  
> 重要说明：本文中的并发指标和阶段目标属于实施建议，当前仓库尚未提供真实生产流量、压测数据或线上运行数据，不能视为已验证结果。

---

## 1. 结论概述

EnglishLover 当前已经具备英语词汇学习产品的核心原型：阶段词库、单词学习、间隔复习、听写、阅读生成、AI 单词解释、学习统计、成就、排行榜、用户资料和管理后台等功能。但当前主要数据仍保存在浏览器 `localStorage` 中，服务端 `front/server.ts` 主要承担 Gemini API 代理和 AI 兜底逻辑，尚未形成真正的后端业务系统。

为了支撑后续立项和实施，推荐将系统演进为：
...
第一阶段不建议直接上复杂微服务。更合理的路线是先建设“模块边界清晰的高性能单体后端”，把用户数据、单词数据、复习数据和管理功能落到数据库中；当 AI 调用、搜索、统计、排行榜等模块出现独立扩展压力时，再逐步拆分为专用服务。
...
当前仓库是一个位于 `front/` 目录下的 React/Vite TypeScript 应用，配有 Express 服务端：

- 前端入口和状态管理集中在 `front/src/App.tsx`。
- 类型定义位于 `front/src/types.ts`。
- 种子词库位于 `front/src/data/words.ts`。
- AI 接口和 Vite/Express 服务位于 `front/server.ts`。
- 主要组件包括 `WordTrainer`、`ReadingModule`、`AdminPanel`、`TodayDashboard`、`AnalyticsDashboard`、`Leaderboard`、`UserProfileInfo` 等。
...
当前项目中的后台能力应在生产化时从“本地口令”升级为服务端 RBAC 权限模型。
